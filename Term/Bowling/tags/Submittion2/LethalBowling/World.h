...
public:
    ...
    static int SolveBoxCollision(Pin& pin1, Pin& pin2, const Vector3& collisionPoint, float distance, Contact& contact);
    static int CalulateNormal(const Box& box, const Vector3& collisionPoint, Vector3& normal);
    static bool IsVertex(const Box& box, const Vector3& point);
    static void FindClosestVertex(const Box& box, const Vector3& point, Vector3& closestVertex, float& distance);
